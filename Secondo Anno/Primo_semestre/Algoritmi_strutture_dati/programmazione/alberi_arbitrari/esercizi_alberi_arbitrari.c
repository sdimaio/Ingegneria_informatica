...
int cammino_arbitrario(nodo* t){
  if(t == NULL)
    return 1;
  if(t->dx == NULL)
    return cammino_arbitrario(t->sx);

return 0;
}

/*int conteggio_nodi_arb(albero t){
  ...
  nodo* x = t->sx;
  while(x != NULL){
    conta = 1 + conteggio_nodi_arb(t->sx);
    x = x->dx;
  }
...
int conta_nodi_prof_arb(albero t, int prof){
  ...
  return 1 + conta_nodi_prof_arb(t->sx, prof-1) + conta_nodi_prof_arb(t->dx, prof);
