...
/* creazione di una pila di dimensioni specificate dall'utente */
...
pila_di_interi*  new_stack(int default_size) {
	...
	/* equivalente a "(*mia_pila).size = default_size;" */	
	mia_pila->A = (int*)calloc(mia_pila->size,sizeof(int));
	mia_pila->top = -1;    /* la pila e' vuota */
...
void push_con_overflow(pila_di_interi* pp, int v) {
	...
	if( pp->top == pp->size - 1 ){
		char messaggio[100];
		sprintf(messaggio,"overflow della pila\0");
		error(messaggio);
	}
	pp->top++;
	pp->A[pp->top] = v;
