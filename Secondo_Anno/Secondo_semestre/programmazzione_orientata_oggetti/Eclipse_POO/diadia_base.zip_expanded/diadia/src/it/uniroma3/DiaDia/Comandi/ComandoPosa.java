...
public class ComandoPosa implements Comando {
	...
	public void esegui(Partita partita) {
		if(partita.ciocatore.getBorsa().hasAttrezzo(attrezzo)) {
			...
			this.io.mostraMessaggio("L'attrezzo cercato non è presente nella borsa");
    ...
    public void esegui(Partita partita) {
        if(partita.ciocatore.getBorsa().hasAttrezzo(attrezzo)) {
            if(partita.getStanzaCorrente().addAttrezzo(partita.ciocatore.getBorsa().removeAttrezzo(attrezzo))) {
                ...
                System.out.println("Qualcosa è andato storto, non è stato posato l'oggetto nella stanza!");
        }
        else
            System.out.println("L'attrezzo cercato non è presente nella borsa!");
    }
	 */
