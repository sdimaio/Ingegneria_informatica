...
import static dist.gui.LettoreImmagini.leggiImmagineMattone;
...
public class GUI extends JPanel implements CostantiGUI {
	...
	public void riportaNelTitolo(int passo, Simulatore simulatore) {
		...
		builder.append(" Persone: "+simulatore.getAmbiente().getAllPersone().size());
	...
	public void paintComponent(Graphics g) {
		...
		for (Persona a : this.simulatore.getPersona()) {
			disegna(g, a);
		}
		
	}

	private void disegna(Graphics g, Persona p) {
        final Coordinate posizione = p.getPosizione();
		final String ids = p.toString();
		disegnaTesto(g, posizione, ids);
		disegnaImmagine(g, p.getImmagine(), posizione, IMMAGINE_SCALA);
	}
